      ...
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column align='center'  label="订单号" width="180">
          <template slot-scope="scope">
            <a class="table_href" :href="'#/spbbhz/sycgddglxq/'+scope.row.order_id">{{scope.row.order_number}}</a>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    fetchOrderList(like_name, start_time, end_time, order_state, pay_type, shop_id, page) {
      adminOrder.adminAppOrderAllList({ like_name, start_time, end_time, order_state, pay_type, shop_id, page }).then(res => {
        const data = res.data
