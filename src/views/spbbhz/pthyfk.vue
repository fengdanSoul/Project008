...
<script>
  ...
  // { feedbackList, feedbackFlag, feedbackDelete }
  export default {
    data() {
      return {
        dialogVisible: false,
    ...
    methods: {
      fectchFeedbackList(start_time, end_time, deal_flag, like_name, page) {
        adminReportForm.feedbackList({ start_time, end_time, deal_flag, like_name, page }).then(response => {
      ...
      handleDelete(id) {
        this.$confirm('确认删除？').then(_ => {
          adminReportForm.feedbackDelete({ id }).then(response => {
            this.$message({
              message: '删除成功',
              type: 'success'
            })
      ...
      handleDealFlag(id) {
        adminReportForm.feedbackFlag({ id, dealFlag: 1 }).then(response => {
          this.$message({
            message: '处理成功',
            type: 'success'
          })
